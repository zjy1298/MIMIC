...
# -*- coding: utf-8 -*-
"""
AC code instrumentation script.
Inserts [TRACE] diagnostic output into accepted_solutions of each enriched QA record,
for subsequent CoT synthesis. Structure based on synthesize_problems.py.
"""
...
def main(args):
    ...
    if len(dataset) == 0:
        print("Error: no solutions to instrument.")
        return
    ...
    system_prompt = (
        ...
        else "You are a code analysis and debugging expert specializing in algorithm instrumentation."
    )
    ...
    # 6. Sampling params (instrumentation needs low temperature to ensure code correctness)
    ...
    for shard_idx in range(args.num_save_shards):
        ...
        inferenced = shard_ds.add_column('instrument_result', responses)
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description="Instrument AC code with [TRACE] diagnostic output"
    )
    ...
    parser.add_argument('--prompt_path', type=str, required=True,
                        help="instrument_code.txt / instrument_code_en.txt")
    ...
    parser.add_argument('--max_solutions_per_qa', type=int, default=None,
                        help="Max solutions per QA to instrument (None = all)")
    ...
    parser.add_argument('--allowed_languages', type=str, nargs='+', default=None,
                        help="Only instrument these languages, e.g. cpp python")
