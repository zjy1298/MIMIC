    ...
    for file_path in file_paths:
        ...
        with open(file_path, "r", encoding="utf-8") as f:
            for line in tqdm(f, desc=f"Reading {os.path.basename(file_path)}"):
                ...
                question_parts = []
                if data.get('title'):
                    question_parts.append(f"### {data['title']}")
                ...
                if data.get('examples'):
                    examples_text = "\n\nExamples:"
                    for i, ex in enumerate(data['examples'][:1], 1):
                        examples_text += f"\nExample {i}:"
                        if ex.get('input'):
                            examples_text += f"\nInput: {ex['input']}"
                        if ex.get('output'):
                            examples_text += f"\nOutput: {ex['output']}"
                    question_parts.append(examples_text)

                question_text = "\n".join(question_parts)
                tags_text = ", ".join(data.get('tags', [])) if data.get('tags') else "None"

                editorial_text = "None"
                if data.get('accepted_solutions') and len(data['accepted_solutions']) > 0:
                    first_solution = data['accepted_solutions'][0]
                    editorial_text = f"Reference code ({first_solution.get('programmingLanguage', 'Unknown')}):\n{first_solution.get('source', 'N/A')}"
...
def inference_worker(args, tokenizer, llm, prompt, system_prompt, sampling_params, examples):
    ...
    chat_texts = [
        ...
        for question, tags, editorial in zip(
            examples['question'], examples['tags'], examples['editorial']
        )
    ]
